...
import { getHealthScoreColor, getHealthScoreColorClass } from '@/features/records/clients/utils/helpers';

interface ClientMetricCardsProps {
  client: Client;
}

export function ClientMetricCards({ client }: Readonly<ClientMetricCardsProps>) {
  const metrics = [
    {
      id: 'providers',
      title: 'PROVIDERS',
                ...
                <div className="flex items-center gap-3">
                  <div className="flex-1">
                    <Progress
                      ...
                      className={cn('h-2', getHealthScoreColorClass(metric.value as number))}
                    />
                  </div>
                  <span className={cn(
